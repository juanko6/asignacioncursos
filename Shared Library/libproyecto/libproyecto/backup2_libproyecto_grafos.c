    ...
    for (int j = 0; j < numCursos; j++) {
        ...
        for (int i = 0; i < numSalas; i++) {
                ...
                for (int k = 0; k <= *numAsignaciones; k++)
		 {
			...
			{
			    if(asignaciones[k].bloque_asignado == cursos[j].bloque)
				 {
...
void imprimirAsignacionesJson(Asignamiento *asignaciones, int numAsignaciones) {
    ...
    for (int i = 0; i < numAsignaciones; i++) {
        ...
        cJSON_AddStringToObject(assignment, "nombre_profesor", asignaciones[i].nombre_profesor);
        cJSON_AddStringToObject(assignment, "nombre_sala", asignaciones[i].nombre_sala);
        cJSON_AddNumberToObject(assignment, "dia_asignado", asignaciones[i].dia_asignado);
        cJSON_AddNumberToObject(assignment, "bloque_asignado", asignaciones[i].bloque_asignado);
...
// aquí se debe llamar a la función que asigna los cursos a las salas
   ...
   // char *asignaciones = asignarCursosASalas(cursos, salas, numCursos, numSalas);
    // printf("\nAsignaciones:\n%s\n", asignacionesJson);
    imprimirAsignacionesJson(asignaciones, numAsignaciones);
