...
JNIEXPORT jstring JNICALL Java_libproyecto_grafos_funcionAsignacion
  (JNIEnv * env, jobject obj, jstring jsonCurso, jstring jsonSala) {
    ...
    if (jsonCursos == NULL) {
        ...
        return (*env)->NewStringUTF(env, ""); // Devolver cadena vacía
    }
    ...
    if (cursos == NULL || salas == NULL) {
        // Manejo de error si no se pueden leer los datos
        (*env)->ReleaseStringUTFChars(env, jsonCurso, jsonCursos);
    ...
    if (asignacionesJson == NULL) {
        // Manejo de error si no se pueden asignar los cursos a las salas
        return (*env)->NewStringUTF(env, ""); // Devolver cadena vacía
    }

    jstring resultado = (*env)->NewStringUTF(env, asignacionesJson);
    free(asignacionesJson); // Liberar memoria después de usar asignacionesJson
    return resultado;
}
